...
const renderNameTitle = (name, color, ccfLevel, title, cardWidth, rightTop, tag) => {
    // 添加安全处理：确保name和tag是有效字符串
    const safeName = String(name || "Luogu User");
    ...
    const tagLength = safeTag ? anafanafo(safeTag)/10*1.2 : 0;
    const nameLength = anafanafo(safeName)/10*1.8;
    const nameColor = NAMECOLOR[color] || NAMECOLOR.Gray;
    const tagSVG = safeTag ? `<defs><filter x="-0.05" y="-0.05" width="1.1" height="1.1" id="tagFilter"><feFlood flood-color="${nameColor}"/><feComposite in="SourceGraphic"/></filter></defs><text filter="url(#tagFilter)" x="${nameLength + (ccfLevel < 3 ? 10 : 28)}" y="-1" fill="white" font-weight="bold" textLength="${tagLength}" font-size="12">${safeTag}</text>` : ``;
    ...
    <g transform="translate(0, 0)" font-family="Verdana, Microsoft Yahei" text-rendering="geometricPrecision">
        ...
        ${tagSVG}
        <text x="${nameLength + (ccfLevel < 3 ? 10 : 28) + (safeTag ? tagLength+5 : 0)}" y="0" class="title" font-weight="normal" font-size="18">
            ${title}
        </text>
 ...
 * @param {number|null} ranking 咕值排名（可能为null）
...
const renderAboutText = (userType, followerCount, followingCount, ranking, slogan, darkmode = false) => {
    ...
    // 修复：正确处理ranking为null的情况
    let rankingText = "咕值排名：";
    if (ranking !== null && ranking !== undefined) {
        rankingText += `第${ranking}名`;
    } else {
        rankingText += "暂无排名";
    }
    let line3 = `<g transform="translate(25,60)"><text x="0" y="15" class="about-text">${rankingText}</text></g>`
